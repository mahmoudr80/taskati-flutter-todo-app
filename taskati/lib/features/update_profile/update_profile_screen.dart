...
class UpdateProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => UserCubit()..loading(),
      child: Scaffold(
        appBar: AppBar(
                  ...
                  BlocBuilder<UserCubit, UserState>(
                    builder: (context, state) {
                      return IconButton(
                        ...
                        icon: CircleAvatar(
                          radius: 50.r,
                          backgroundImage: state.currentUser == null
                      ...
                      builder: (context, state) {
                        return IconButton(
                          ...
                          icon: Container(
                            ...
                            child: Icon(Icons.camera_alt, color: Colors.indigo),
                          ),
                        );
                      },
                    ),
                  ...
                  BlocBuilder<UserCubit, UserState>(
                    builder: (context, state) {
                      return Container(
                        ...
                        child: IconButton(
                          onPressed: () async {
                            final newName = await renameBottomSheet(
                              context,
                              state.currentUser!.name,
                            );
                            final user = await UserServices.getUser();
